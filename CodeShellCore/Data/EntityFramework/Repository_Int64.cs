...
namespace CodeShellCore.Data.EntityFramework
{
        ...
        where TContext : DbContext
    {
        ...
        public virtual IQueryable<Named<long>> QueryNamed(IQueryable<T> q = null)
        {
            q = q ?? Loader;
            if (typeof(T).Implements(typeof(INamed<long>)))
            {
                return ((IQueryable<INamed<long>>)q).Select(d => new Named<long> { Id = d.Id, Name = d.Name });
            }
            else
            {
                return q.Select(d => new Named<long> { Id = d.Id, Name = "(" + d.Id + ")" });
            }
        }
