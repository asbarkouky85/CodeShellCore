...
export abstract class TenantCreateBase extends DTOEditComponentBase {
  ...
  async LoadLookupsAsync(opts: any): Promise<any> {
    var lookups = await this.Service.GetEditLookups(opts);
  ...
  constructor(rt: ActivatedRoute, res: Injector) {
    ...
    this.Listener.SendMessage.subscribe((d: any) => {
      if (this.OutputListener) this.OutputListener.AddMessage(d);
    });
  ...
  OnReady() {
    ...
    if (this.Lookups.tenants && this.modelId) {
      this.Lookups.tenants = this.Lookups.tenants.filter(
        (e) => e.id != this.modelId
      );
    }
  }
  ...
  canRender() {
    if (this.model.entity.parentId && this.model.entity.useParentUI) {
      return false;
    }
    return true;
  }
  ...
  SyncTenant() {
    if (this.OutputListener) this.OutputListener.Clear();
  ...
  Submit() {
    if (this.IsNew && this.model.dbName) {
      ...
      this.Listener.Start().then((d) => {
        ...
        this.Sql.CreateTenantDatabase(req)
          .then((d) => {
