...
namespace CodeShellCore.Moldster.Services.Json
{
    public class JsonDataService : ServiceBase, IDataService, IJsonConfigProvider
    {
        ...
        public JsonDataService(IPathsService prov)
        {
            Paths = prov;
        }
        ...
        public PageRenderDTO[] GetDomainPagesForRendering(string mod, string domain)
        {
            return GetNgModule(mod).GetDomain(domain).Pages.Select(d => new PageRenderDTO { ViewPath = d.ViewPath }).ToArray();
        }
        ...
        public PageOptions GetPageOptionsById(long id)
        {
            throw new NotImplementedException();
        }
