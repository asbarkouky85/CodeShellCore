...
namespace CodeShellCore.Moldster
{
    public class DefaultPathsService : IPathsService,ILayoutsService
    {
        ...
        public virtual List<MoldsterEnvironment> GetEnvironments()
        {
            ...
            if (!f.TryRead(out List<MoldsterEnvironment> _envs))
        ...
        public DefaultPathsService()
        {
            ...
            var settings = Environment.GetEnvironmentVariable(ConfigNames.CODESHELL_SETTINGS_PATH) ?? ".";
            _filePath = Path.Combine(settings, "appEnvironments.json");
            if (settings != null)
            {
                sol = settings.Replace("\\", "/").GetBeforeLast("/");
            }
